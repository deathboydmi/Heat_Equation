...
public:
  ...
  std::vector<double> implicit_method() {
    std::vector<std::vector<double>> grid(n_t);
    std::vector<double> rod(n_x);
    ...
    grid.push_back(rod);
      ...
      // tridiagonal matrix
    std::vector<std::vector<double>> trid_matr(n_x - 2);
    init_trid_matr(trid_matr);
    
    std::vector<double> f_x(n_x - 2);
    current_x = dx;
    ...
    for (size_t i = 0; i < n_t; ++i) {
      std::vector<double> free_vals(n_x - 2);
